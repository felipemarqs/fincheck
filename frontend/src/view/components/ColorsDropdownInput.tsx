...
export const ColorsDropdownInput = ({ error, className }: ColorsDropdownInputProps) => {
    ...
    <div>
      <DropdownMenu.Root>
        <DropdownMenu.Trigger>
          <button
            className={cn(
              "bg-white rounded-lg w-full border border-gray-500 px-3 h-[52px] text-gray-700 focus:border-gray-800 transition-all outline-none text-left relative",
        ...
        <DropdownMenu.Content className="grid grid-cols-4">
          {colors.map((color) => (
            <DropdownMenu.Item onSelect={() => handleSelect(color)} key={color.color}>
